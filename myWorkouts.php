...
$getLimit= "SELECT * FROM users u INNER JOIN workout w INNER JOIN owner o ON u.id = o.user_id AND w.id = o.workout_id
WHERE u.email = '$email'";
...
xmlhttp.open("GET","myworkoutDisplay.php?change="+change,true);
xmlhttp.send();
}
</script>
</head>
<title>SWIMDUB - My Workouts</title>
<body bgcolor=black>
<?PHP
include("header.php");
include("db_connect.php");
...
WHERE u.email = '$email' ORDER BY date_added DESC LIMIT 0, 5";
